...
// Navigation groupée : Pilotage / Activité / Contenu / Marketing / Système.
const NAV_GROUPS = [
    ...
    { label: 'Activité', items: [
        ...
        { name: 'Console ventes',  href: '/admin/sales',          icon: 'M14.25 7.756a4.5 4.5 0 100 8.488M7.5 10.5h5.25m-5.25 3h5.25M21 12a9 9 0 11-18 0 9 9 0 0118 0z', adminOnly: true },
    ]},
    { label: 'Contenu', items: [
...
// Groupes filtrés par rôle ; un groupe vide (ex : Contenu pour un pro) disparaît.
const navGroups = computed(() => NAV_GROUPS
    .map(g => ({ ...g, items: g.items.filter(item => !item.adminOnly || isAdmin.value) }))
    .filter(g => g.items.length))

// Liste à plat (fil d'ariane, état actif)
...
<template>
    <div class="min-h-screen flex" style="background: var(--bg-base)">
        ...
        <aside class="adm-sidebar w-64 flex-shrink-0 flex flex-col">
            ...
            <!-- Nav groupée : Pilotage / Activité / Contenu / Marketing / Système -->
            <nav class="flex-1 overflow-y-auto py-4 px-3">
                <div v-for="group in navGroups" :key="group.label" class="adm-nav-group">
