...
// FE-01 — guard test vide : évite currentIndex = -1 quand allQuestions est vide
const currentIndex = ref(
    allQuestions.value.length > 0
        ? Math.min(startIndex, allQuestions.value.length - 1)
        : 0
)
const currentQuestion = computed(() => allQuestions.value[currentIndex.value])
const totalQuestions = computed(() => allQuestions.value.length)
const percent = computed(() => ((currentIndex.value + 1) / totalQuestions.value) * 100)

const value = ref(null)
const startedAt = ref(Date.now())

// FE-02 — empêche double-soumission
        ...
        <div class="max-w-2xl mx-auto">
            ...
            <template v-if="totalQuestions === 0">
                <div class="pt-card p-8 text-center text-slate-500">
                    <p class="font-medium">Ce test ne contient aucune question pour l'instant.</p>
                    <Link :href="route('tests.index')" class="pt-btn-ghost mt-4 inline-block text-sm">← Retour aux tests</Link>
                </div>
            </template>
            <template v-else>
