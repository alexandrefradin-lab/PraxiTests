...
class Questions
{
    ...
    // ── LOGIQUE (niveau expert) — déplacement de figure (chrono 55 s) ─────
    private static function logiqueHard(): array
    {
        $sec = 'Raisonnement logique';
        $t   = 55;

        return [
            // L11 — un point se déplace d'une case dans le sens horaire sur le
            //       pourtour : (0,0),(0,1),(0,2) → (1,2). Correct slot 2.
            [
                'section' => $sec, 'type' => 'single',
                'prompt'  => 'Le point se déplace d\'une case à chaque étape. Où se trouve-t-il ensuite ?',
                'meta'    => ['time_limit' => $t, 'figure' => self::strip([
                    self::gridB([[0, 0]]), self::gridB([[0, 1]]), self::gridB([[0, 2]]),
                ])],
                'options' => [
                    self::figOpt(1, 'A', self::gridB([[0, 0]])),
                    self::figOpt(2, 'B', self::gridB([[1, 2]])),
                    self::figOpt(3, 'C', self::gridB([[2, 2]])),
                    self::figOpt(4, 'D', self::gridB([[1, 1]])),
                ],
                'scoring' => ['dimension' => 'logique', 'correct' => 2],
            ],

            // L12 — le point saute DEUX cases dans le sens horaire sur le pourtour :
            //       (0,0),(0,2),(2,2) → (2,0). Correct slot 3.
            [
                'section' => $sec, 'type' => 'single',
                'prompt'  => 'Le point saute de deux cases à chaque étape, en tournant. Où atterrit-il ensuite ?',
                'meta'    => ['time_limit' => $t, 'figure' => self::strip([
                    self::gridB([[0, 0]]), self::gridB([[0, 2]]), self::gridB([[2, 2]]),
                ])],
                'options' => [
                    self::figOpt(1, 'A', self::gridB([[2, 2]])),
                    self::figOpt(2, 'B', self::gridB([[1, 0]])),
                    self::figOpt(3, 'C', self::gridB([[2, 0]])),
                    self::figOpt(4, 'D', self::gridB([[0, 0]])),
                ],
                'scoring' => ['dimension' => 'logique', 'correct' => 3],
            ],
        ];
    ...
    // ── VERBAL (niveau expert) — vocabulaire rare & abstraction ───────────
    private static function verbalHard(): array
    {
        $sec = 'Raisonnement verbal';
        $t   = 40;

        return [
            // V11 — synonyme savant : sagacité ≈ perspicacité (piège : sagesse). Correct slot 4.
            [
                'section' => $sec, 'type' => 'single', 'meta' => ['time_limit' => $t],
                'prompt'  => 'Quel mot est le plus proche du sens de « SAGACITÉ » ?',
                'options' => [
                    ['value' => 1, 'label' => 'Sagesse'],
                    ['value' => 2, 'label' => 'Lenteur'],
                    ['value' => 3, 'label' => 'Gentillesse'],
                    ['value' => 4, 'label' => 'Perspicacité'],
                ],
                'scoring' => ['dimension' => 'verbal', 'correct' => 4],
            ],
            // V12 — analogie d'opposés abstraits : abondance↔rareté, richesse↔pauvreté
            //       (piège : « manque »). Correct slot 2.
            [
                'section' => $sec, 'type' => 'single', 'meta' => ['time_limit' => $t],
                'prompt'  => 'ABONDANCE est à RARETÉ ce que RICHESSE est à… ?',
                'options' => [
                    ['value' => 1, 'label' => 'Argent'],
                    ['value' => 2, 'label' => 'Pauvreté'],
                    ['value' => 3, 'label' => 'Économie'],
                    ['value' => 4, 'label' => 'Manque'],
                ],
                'scoring' => ['dimension' => 'verbal', 'correct' => 2],
            ],
        ];
    ...
    // ── NUMÉRIQUE (niveau expert) — différences de second ordre ───────────
    private static function numeriqueHard(): array
    {
        $sec = 'Raisonnement numérique';
        $t   = 45;

        return [
            // N11 — écarts en nombres impairs (+1,+3,+5,+7,+9) : 1,2,5,10,17 → 26. Correct slot 2.
            [
                'section' => $sec, 'type' => 'single', 'meta' => ['time_limit' => $t],
                'prompt'  => 'Quel nombre complète la suite : 1, 2, 5, 10, 17, … ?',
                'options' => [
                    ['value' => 1, 'label' => '24'],
                    ['value' => 2, 'label' => '26'],
                    ['value' => 3, 'label' => '28'],
            ...
            // N12 — écarts croissants (+1,+2,+3,+4,+5,+6) : 3,4,6,9,13,18 → 24. Correct slot 3.
            [
                'section' => $sec, 'type' => 'single', 'meta' => ['time_limit' => $t],
                'prompt'  => 'Quel nombre complète la suite : 3, 4, 6, 9, 13, 18, … ?',
                'options' => [
                    ['value' => 1, 'label' => '22'],
                    ['value' => 2, 'label' => '23'],
                    ['value' => 3, 'label' => '24'],
    ...
    // ── SPATIAL (niveau expert) — transformations de formes denses (5 cases) ─
    private static function spatialHard(): array
    {
        $sec = 'Raisonnement spatial';
        $t   = 60;

        return [
            // S11 — rotation 90° horaire d'une forme en Γ (5 cases).
            //   Réf {(0,0),(0,1),(0,2),(1,0),(2,0)} → {(0,0),(0,1),(0,2),(1,2),(2,2)}. Correct slot 3.
            [
                'section' => $sec, 'type' => 'single',
                'prompt'  => 'Quelle grille correspond à la figure ci-dessus après un quart de tour vers la DROITE (90° horaire) ?',
                'meta'    => ['time_limit' => $t, 'figure' => self::wrap(self::gridB([[0, 0], [0, 1], [0, 2], [1, 0], [2, 0]]))],
                'options' => [
                    self::figOpt(1, 'A', self::gridB([[0, 0], [0, 1], [0, 2], [1, 0], [2, 0]])),   // identique
                    self::figOpt(2, 'B', self::gridB([[2, 2], [2, 1], [2, 0], [1, 2], [0, 2]])),   // rotation 180
                    self::figOpt(3, 'C', self::gridB([[0, 0], [0, 1], [0, 2], [1, 2], [2, 2]])),   // ✓ 90° horaire
                    self::figOpt(4, 'D', self::gridB([[2, 0], [1, 0], [0, 0], [2, 1], [2, 2]])),   // 90° anti-horaire
                ],
                'scoring' => ['dimension' => 'spatial', 'correct' => 3],
            ],

            // S12 — miroir vertical d'une forme asymétrique (5 cases).
            //   Réf {(0,0),(0,1),(1,0),(2,0),(2,2)} → {(0,1),(0,2),(1,2),(2,0),(2,2)}. Correct slot 4.
            [
                'section' => $sec, 'type' => 'single',
                'prompt'  => 'Quelle grille est l\'image miroir de la figure ci-dessus (symétrie par un axe VERTICAL) ?',
                'meta'    => ['time_limit' => $t, 'figure' => self::wrap(self::gridB([[0, 0], [0, 1], [1, 0], [2, 0], [2, 2]]))],
                'options' => [
                    self::figOpt(1, 'A', self::gridB([[0, 0], [0, 1], [1, 0], [2, 0], [2, 2]])),   // identique
                    self::figOpt(2, 'B', self::gridB([[2, 0], [2, 1], [1, 0], [0, 0], [0, 2]])),   // miroir horizontal
                    self::figOpt(3, 'C', self::gridB([[2, 2], [2, 1], [1, 2], [0, 2], [0, 0]])),   // rotation 180
                    self::figOpt(4, 'D', self::gridB([[0, 1], [0, 2], [1, 2], [2, 2], [2, 0]])),   // ✓ miroir vertical
                ],
                'scoring' => ['dimension' => 'spatial', 'correct' => 4],
            ],
        ];
