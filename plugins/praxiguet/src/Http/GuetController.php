...
namespace Praxis\Plugins\PraxiGuet\Http;
...
use Praxis\Plugins\PraxiGuet\Data\Levels;
use Praxis\Plugins\PraxiGuet\Data\Notions;
use Praxis\Plugins\PraxiGuet\Data\Prompts;
use Praxis\Plugins\PraxiGuet\Models\GuetLevelProgress;
use Praxis\Plugins\PraxiGuet\Models\GuetNotionProgress;
use Praxis\Plugins\PraxiGuet\Models\GuetProfile;
 ...
 */
class GuetController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($redirect = $this->rewards->unlockRedirect('praxiguet.index', $user)) {
            return $redirect;
        }

        $profile = GuetProfile::forUser($user->id);

        $progress = GuetNotionProgress::forUser($user->id)->get()->keyBy('notion_id');
        $levels   = GuetLevelProgress::forUser($user->id)->get()->keyBy('level');
        ...
        return Inertia::render('PraxiGuetIndex', [
            'appDescription' => $this->rewards->descriptionFor('praxiguet'),
            ...
            'config' => [
                ...
                'intervals'   => GuetNotionProgress::INTERVALS,
                'max_box'     => GuetNotionProgress::MAX_BOX,
            ],
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($user, $data, $level, $profile, $passed, &$awarded) {
            ...
            foreach ($data['notions'] ?? [] as $seen) {
                ...
                $row = GuetNotionProgress::firstOrNew([
                    'user_id'   => $user->id,
                    'notion_id' => $seen['id'],
                ]);
            ...
            if ($passed) {
                $row = GuetLevelProgress::firstOrNew([
                    'user_id' => $user->id,
                    'level'   => $level['id'],
                ]);
    ...
    {
        $total = Notions::count() * GuetNotionProgress::MAX_BOX;
